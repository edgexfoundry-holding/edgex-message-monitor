...
func dataScreen(win fyne.Window, appManager *services.AppManager) fyne.CanvasObject {
	...
	if appManager.GetConfig().GetCurrentProcotol() == config.ProtocolRedis {
		host, port = appManager.GetRedisHostPort()
	} else {
		host, port = appManager.GetMqttHostPort()
	}
	...
	disconnectedContent := container.NewCenter(container.NewVBox(
		widget.NewCard("You are currently disconnected from EdgeX Foundry",
			fmt.Sprintf("Would you like to connect to %v with %v:%d?", appManager.GetConfig().GetCurrentProcotol(), host, port),
			container.NewCenter(
	...
	searchBox := container.NewVBox(
		widget.NewLabelWithStyle("Filter (loosely search)", fyne.TextAlignLeading, fyne.TextStyle{Bold: true}),
	...
	bufferSizeContainer := container.NewGridWithColumns(2,
		container.NewBorder(nil, nil, container.NewHBox(h.playIcon, widget.NewLabel("Buffer size")), h.applyBufferSizeBtn, h.bufferSize),
		h.bufferProgress,
	)
