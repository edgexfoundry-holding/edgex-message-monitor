...
type dataPageHandler struct {
	...
	parentTopic   *widget.Label
	topic         *widget.Entry
	topicBtn      *widget.Button
	resetTopicBtn *widget.Button
...
func NewDataPageHandler(appState *services.AppManager) *dataPageHandler {
	...
	p.parentTopic = widget.NewLabel("")

	p.topic = widget.NewEntry()
	p.topic.SetPlaceHolder("Topic cannot be empty")
	p.topic.Validator = func(s string) error {

		err := data.StringNotEmptyValidator(s)
		if err != nil {
			return err
		}

		preferences := fyne.CurrentApp().Preferences()
		prefix := strings.TrimRight(preferences.StringWithFallback(config.PrefEventsTopic, config.DefaultEventsTopic), "#")
		return data.MustGenerateValidRegexValidator(prefix, s)
	}

	p.topicBtn = widget.NewButtonWithIcon("", theme.ComputerIcon(), func() {})
	p.resetTopicBtn = widget.NewButtonWithIcon("", theme.DeleteIcon(), func() {})
...
func (p *dataPageHandler) RehydrateSession() {
	...
	parentTopic := preferences.StringWithFallback(config.PrefEventsTopic, config.DefaultEventsTopic)
	if !strings.HasSuffix(parentTopic, "#") {
		p.topic.Hide()
		p.topicBtn.Hide()
		p.resetTopicBtn.Hide()
	} else {
		p.topic.Show()
		p.topicBtn.Show()
		p.resetTopicBtn.Show()
	}
	t := strings.TrimRight(parentTopic, "#")
	p.parentTopic.SetText(t)

	topic := p.appState.GetDataPageSelectedTopic()
	if topic != nil {
		p.topic.SetText(config.StringVal(topic))
	} else {
		p.topic.SetText("#")
	}
...
func (p *dataPageHandler) SetupBindings() {
	...
	p.topic.OnChanged = func(s string) {
		if strings.Trim(s, " ") != "" {
			p.topicBtn.Enable()
			p.resetTopicBtn.Enable()
		} else {
			p.topicBtn.Disable()
			p.resetTopicBtn.Disable()
		}
	}
	p.topic.OnSubmitted = func(s string) {
	...
	p.topicBtn.OnTapped = func() {
		p.appState.SetDataPageSelectedTopic(p.topic.Text)
	...
	p.resetTopicBtn.OnTapped = func() {
		p.appState.SetDataPageSelectedTopic("#")
		p.topic.SetText("#")
...
func (p *dataPageHandler) Refresh() {
	if p.topic.Validate() == nil {
		p.appState.SetDataPageSelectedTopic(p.topic.Text)
	}
...
func (p *dataPageHandler) updateStatusByDataType(currentDataType string) {
	log.Debugf("dataPageHandler: updateStatusByDataType %v", currentDataType)
...
func (p *dataPageHandler) updateTableByDataType(currentDataType string) {
	log.Debugf("dataPageHandler: updateTableByDataType %v", currentDataType)
...
func (p *dataPageHandler) OnEventReceived(event messaging.Envelope) {
