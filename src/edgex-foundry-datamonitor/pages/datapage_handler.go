...
func NewDataPageHandler(appState *services.AppManager) *dataPageHandler {
	...
	// start/paus button
	playIcon := widget.NewButtonWithIcon("", theme.MediaStopIcon(), p.onPlayOrPause)
	p.playIcon = playIcon
	...
	p.search.SetPlaceHolder("Pause table data to search")
	p.search.Disable()
	...
	// p.bufferSizeBinding.Set(fyne.CurrentApp().Preferences().IntWithFallback(config.PrefBufferSizeInDataPage, config.DefaultBufferSizeInDataPage))
...
func (p *dataPageHandler) SetupBindings() {
	...
	p.searchBtn.OnTapped = func() {
		...
		p.table.Refresh() // data binding refresh is slow, so explicitly refresh table
	}
...
func (p *dataPageHandler) onPlayOrPause() {

	p.appState.GetDB().BackupCurrentData()

	if !p.pauseUi {
		p.playIcon.SetIcon(theme.MediaPlayIcon())
		p.search.Enable()
	} else {
		p.playIcon.SetIcon(theme.MediaStopIcon())
		p.search.Disable()
		p.search.SetText("")
		p.resetSearchBtn.Disable()
...
func (p *dataPageHandler) renderReadingsTable() *widget.Table {
	t := widget.NewTable(
		func() (int, int) {
			p.tableDataLock.RLock()
			defer p.tableDataLock.RUnlock()
			return len(*p.readingsTableDataMapBinding) + 1, 11
		},
		...
		func(i widget.TableCellID, o fyne.CanvasObject) {
			...
			default:
				...
				case 10:
					topic, _ := row.GetItem("Topic")
					v, _ := topic.(binding.String).Get()
...
func (p *dataPageHandler) renderEventsTable() *widget.Table {
	t := widget.NewTable(
		func() (int, int) {
			p.tableDataLock.RLock()
			defer p.tableDataLock.RUnlock()
			return len(*p.eventsTableDataMapBinding) + 1, 8
		},
		func() fyne.CanvasObject {
			return widget.NewLabel("---fdaec17c-c0fc-4a04-982e-31a08a0bb776---")
		},
...
func (p *dataPageHandler) setBufferUsageBindingByDataType(dataType string) {
	switch dataType {
	case config.DataTypeEvents:
		p.bufferUsageBinding = binding.BindFloat(config.Float(float64(p.appState.GetDB().GetTotalEventsCount(false))))
	case config.DataTypeReadings:
		p.bufferUsageBinding = binding.BindFloat(config.Float(float64(p.appState.GetDB().GetTotalReadingsCount(false))))
	}
}
...
func (p *dataPageHandler) updateBufferUsageBindingByDataType(currentDataType string) {
	...
	switch currentDataType {
	case config.DataTypeEvents:
		p.bufferUsageBinding.Set(float64(p.appState.GetDB().GetTotalEventsCount(false)))
	case config.DataTypeReadings:
		p.bufferUsageBinding.Set(float64(p.appState.GetDB().GetTotalReadingsCount(false)))
	}
	p.bufferProgress.Bind(p.bufferUsageBinding)
...
func (p *dataPageHandler) updateStatusByDataType(currentDataType string) {
	...
	switch currentDataType {
	case config.DataTypeEvents:
		rowCount = int(p.appState.GetDB().GetEventsCount(p.pauseUi))
		recordType = "events"
	case config.DataTypeReadings:
		rowCount = int(p.appState.GetDB().GetReadingsCount(p.pauseUi))
		recordType = "readings"
	}
...
func (p *dataPageHandler) updateTableByDataType(currentDataType string) {
	...
	if currentDataType == config.DataTypeEvents {
		...
		events := db.GetEvents(p.pauseUi)

		evts := make([]appTypes.MessageEvent, len(events))
		copy(evts, events)
		...
		readings := db.GetReadings(p.pauseUi)

		rdngs := make([]appTypes.MessageReading, len(readings))
		copy(rdngs, readings)
...
func (p *dataPageHandler) OnEventReceived(event appTypes.MessageEvent) {
