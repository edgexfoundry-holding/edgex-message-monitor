...
type EventProcessor struct {
	eventsChannel <-chan messaging.Envelope
...
func NewEventProcessor(eventsChannel chan messaging.Envelope) *EventProcessor {
...
func (ep *EventProcessor) Deactivate() {
	...
	defer ep.Unlock()
	ep.state <- Paused
	ep.CurrentState = Paused
}

func (ep *EventProcessor) GetState() ProcessorState {
	ep.RLock()
	defer ep.RUnlock()
	return ep.CurrentState
}
...
func (ep *EventProcessor) processEvent(envelope messaging.Envelope) {

	for _, listener := range ep.eventListeners {
		listener.OnEventReceived(envelope)
	}
...
type EventListener interface {
	OnEventReceived(event messaging.Envelope)
}
