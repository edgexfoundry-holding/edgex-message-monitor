...
type EventProcessor struct {
	eventsChannel <-chan *appTypes.MessageEvent
...
func NewEventProcessor(eventsChannel chan *appTypes.MessageEvent) *EventProcessor {
...
func (ep *EventProcessor) processEvent(event *appTypes.MessageEvent) {
...
func (t *topNEvents) Add(e *appTypes.MessageEvent) {
	if len(t.events) == t.n {
		t.events = t.events[1:]
	}
...
func (l *topNEvents) Get() []*appTypes.MessageEvent {
	return l.events
}
...
type shortMemoryEventsSlicer interface {
	Add(e *appTypes.MessageEvent)
	Get() []*appTypes.MessageEvent
	GetJson() string
}

type EventListeners []EventListener
type EventListener interface {
	OnEventReceived(event appTypes.MessageEvent)
}
