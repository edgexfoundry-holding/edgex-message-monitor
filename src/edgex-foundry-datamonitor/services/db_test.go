...
func Test_IngestEvents(t *testing.T) {
	...
	mockPreferences.On("StringWithFallback", config.PrefMQTTHost, "localhost").Return("localhost")
	mockPreferences.On("IntWithFallback", config.PrefMQTTPort, 1883).Return(1883)
	mockPreferences.On("StringWithFallback", config.PrefMQTTUsername, "").Return("")
	mockPreferences.On("StringWithFallback", config.PrefMQTTPassword, "").Return("")
	mockPreferences.On("StringWithFallback", config.PrefMQTTClientId, "").Return("TestClient_Subscribe_MQTT")
	mockPreferences.On("StringWithFallback", config.PrefEventsTopic, config.DefaultEventsTopic).Return(config.DefaultEventsTopic)

	mockPreferences.On("StringWithFallback", config.PrefSelectedMessaging, string(config.DefaultSelectedMessaging)).
		Return(string("MQTT"))

	mockApp.On("Preferences").Return(mockPreferences)

	db := NewDB(mockPreferences)

	db.OnEventReceived(dummyEvent())

	db.updateTopicFilter("#")
...
func dummyEvent() messaging.Envelope {
	return messaging.Envelope{
		Serial:  0,
		EventId: "event_id",
		Topic:   "edgex/events/device/dummy_device/profile/resource",
		Content: dtos.Event{
...
func interestingEvent() messaging.Envelope {

	return messaging.Envelope{
		Serial:  0,
		EventId: "interesting_id",
		Topic:   "edgex/events/device/interesting_device/interesting_profile/interesting_resource",
		Content: dtos.Event{
			Versionable: common.Versionable{},
			Id:          "interesting_id",
