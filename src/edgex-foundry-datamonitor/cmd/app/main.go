...
func main() {
	...
	events := make(chan *appTypes.MessageEvent, config.MaxBufferSize)
	...
	settingsPageHandler := pages.NewSettingsPageHandler(AppManager)
	AppManager.SetPageHandler(pages.SettingsPageKey, settingsPageHandler)

	go ep.Run()

	client.OnConnect = func() bool {
		messages, errs := client.Subscribe(cfg.GetDefaultTopic())
