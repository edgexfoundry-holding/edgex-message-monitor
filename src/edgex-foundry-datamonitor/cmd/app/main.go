...
func main() {
	...
	events := make(chan messaging.Envelope, config.MaxBufferSize)
	...
	db := services.NewDB(a.Preferences())
	...
	client.OnConnect = func() bool {
		...
		messages, errs := client.Subscribe(cfg.GetTopic())
			...
			for {
				...
				case err := <-errs:
					if client.IsConnecting {
						...
						uerr := errors.New("Error while subscribing to message bus")
						dialog.ShowError(uerr, topWindow)
						log.Error(err)
	...
	setPage := func(uid widget.TreeNodeID, t pages.Page, appMgr *services.AppManager) {
		...
		pageHandler := appMgr.GetPageHandler(uid)
		if pageHandler != nil {
			pageHandler.Refresh()
		}
	}
...
func makeNav(setPage func(_ widget.TreeNodeID, page pages.Page, appMgr *services.AppManager), appMgr *services.AppManager) fyne.CanvasObject {
	...
	statusLabel := widget.NewLabelWithStyle("", fyne.TextAlignCenter, fyne.TextStyle{Italic: true, Bold: true})
	statusPb := widget.NewProgressBarInfinite()

	playPauseBtn := widget.NewButtonWithIcon("", theme.MediaPauseIcon(), func() {

		processorState := appMgr.GetProcessorState()
		db := appMgr.GetDB()

		switch processorState {
		case services.Running:
			statusLabel.Text = "RUNNING"
			appMgr.GetEventProcessor().Deactivate()
			db.Pause()
		default:
			statusLabel.Text = "PAUSED"
			appMgr.GetEventProcessor().Activate()
			db.Resume()
		}
		appMgr.Refresh()
	})
