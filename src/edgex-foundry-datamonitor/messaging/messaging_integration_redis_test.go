...
func TestClient_Subscribe_Redis(t *testing.T) {
	type args struct {
		topic string
	}
	tests := []struct {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			mockPreferences.On("StringWithFallback", config.PrefRedisHost, "localhost").Return("localhost")
			mockPreferences.On("IntWithFallback", config.PrefRedisPort, 6379).Return(6379)
			mockPreferences.On("StringWithFallback", config.PrefRedisPassword, "").Return("")

			mockPreferences.On("StringWithFallback", config.PrefSelectedMessaging, string(config.DefaultSelectedMessaging)).
				Return(string(config.DefaultSelectedMessaging))
