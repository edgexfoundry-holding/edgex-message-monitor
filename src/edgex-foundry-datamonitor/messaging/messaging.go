...
func (c *Client) Connect() error {
	...
	messageBus, err := edgexM.NewMessageClient(c.getMessageBusConfig())
...
func (c *Client) getMessageBusConfig() types.MessageBusConfig {

	var mbc types.MessageBusConfig

	if c.cfg.GetSelectedMessaging() == config.RedisMessagingType {
		mbc = types.MessageBusConfig{
		...
		mbc.Optional = map[string]string{}
		if c.cfg.GetRedisPassword() != "" {
			mbc.Optional["Password"] = c.cfg.GetRedisPassword()
		}
	} else {
		mbc = types.MessageBusConfig{
			SubscribeHost: types.HostInfo{
				...
				Protocol: edgexM.MQTT,
			},
			Type: edgexM.MQTT,
		}
		mbc.Optional = map[string]string{}
		if c.cfg.GetMQTTClientId() != "" {
			mbc.Optional["ClientId"] = c.cfg.GetMQTTClientId()
		}
		if c.cfg.GetMQTTUsername() != "" {
			mbc.Optional["Username"] = c.cfg.GetMQTTUsername()
		}
		if c.cfg.GetMQTTPassword() != "" {
			mbc.Optional["Password"] = c.cfg.GetMQTTPassword()
		}
	}
