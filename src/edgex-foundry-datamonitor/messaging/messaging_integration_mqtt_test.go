...
//go:build integration_mqtt
// +build integration_mqtt
...
func TestClient_Subscribe_MQTT(t *testing.T) {
	type args struct {
		topic string
	}
	tests := []struct {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			events := make(chan messaging.Envelope)
			...
			for {
				...
				case msgEnvelope := <-messages:
					...
					events <- messaging.Envelope{
						Topic:   msgEnvelope.ReceivedTopic,
						EventId: event.Id,
						Content: *event,
					}
